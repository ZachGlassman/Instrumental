...
def _cameras():
    ...
    cams = []
    num = INT()
    if lib.is_GetNumberOfCameras(byref(num)) == 0:
        if num.value >= 1:
            cam_list = create_camera_list(num)()
            cam_list.dwCount = ULONG(num.value)  # This is stupid

            if lib.is_GetCameraList(pointer(cam_list)) == 0:
                ids = []
                repeated = []
...
class UC480_Camera(Camera):
    ...
    def _get_sensor_color_mode(self):
        ...
        sInfo = SENSORINFO()
        lib.is_GetSensorInfo(self._hcam, byref(sInfo))
        return int(binascii.b2a_hex(sInfo.nColorMode), 16)
    ...
    def get_captured_image(self, timeout='1s', copy=True):
        ...
        for ptr in mem_ptrs:
            ...
            array = self._array_from_buffer(memoryview(buf_ptr.contents))
